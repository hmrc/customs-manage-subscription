...
class TaxEnrolmentsServiceSpec extends BaseSpec {
  ...
  "SubscriptionTaxEnrolmentIntegrationService" should {
    ...
    "store recipient details in store and call tax-enrolments connector" in {
      when(mockContactDetailsStore.saveRecipientDetailsForBundleId(anyString, any[Option[Eori]], any[RecipientDetails], anyString, anyString)).thenReturn(true)
    ...
    "fail when calling tax-enrolments fails" in {
      when(mockContactDetailsStore.saveRecipientDetailsForBundleId(anyString, any[Option[Eori]], any[RecipientDetails], anyString, anyString)).thenReturn(false)
