...
import uk.gov.hmrc.customs.managesubscription.domain.{RcmNotificationRequest, RecipientDetails, SubscriptionCompleteStatus}
...
class EmailServiceSpec extends BaseSpec {
  ...
  private val rcmTemplateId = "customs_rcm_notifications"
  ...
  private val gyeRecipientDetails = RecipientDetails("GetYourEORI", gyeRecipientEmailAddress, gyeRecipientFullName, Some(gyeOrgName), Some(gyeCompletionDate))
  ...
  private val migrateRecipientDetails = RecipientDetails("Migrate", migrateRecipientEmailAddress, migrateRecipientFullName, Some(migrateOrgName), Some(migrateCompletionDate))

  private val emailAddress = "john.doe@example.com"
  val rcmNotifyRequest = RcmNotificationRequest("a@example.com","fullname","ZZ123456789000","26-May-2019 08:12:83")

  private val notifyEmail = Email(
    to = List(emailAddress),
    templateId = rcmTemplateId,
    parameters = Map("email" -> "a@example.com", "name" -> "fullname", "eori" -> "ZZ123456789000", "timestamp" -> "26-May-2019 08:12:83")
  )
  ...
  "EmailService" should {
    "call emailConnector with proper content for GetYourEORI success email" in {
    ...
    "call emailConnector with proper content for GetYourEORI not success email" in {
      when(mockEmailConnector.sendEmail(any[Email])(any[HeaderCarrier])).thenReturn(Future.successful(HttpResponse(200, "")))
    ...
    "call emailConnector with proper content for Migrate success email" in {
      when(mockEmailConnector.sendEmail(any[Email])(any[HeaderCarrier])).thenReturn(Future.successful(HttpResponse(200, "")))
    ...
    "call emailConnector with proper content for Migrate not success email" in {
      when(mockEmailConnector.sendEmail(any[Email])(any[HeaderCarrier])).thenReturn(Future.successful(HttpResponse(200, "")))
    ...
    "call emailConnector with proper content for notifying RCM about the exception" in {

      when(mockEmailConnector.sendEmail(any[Email])(any[HeaderCarrier])).thenReturn(Future.successful(HttpResponse(200, "")))

      emailService.sendEmail(rcmNotifyRequest)

      verify(mockEmailConnector).sendEmail(meq(notifyEmail))(meq(hc))
    }
