...
@Singleton
class SubscriptionCompleteBusinessService @Inject()(recipientDetailsStore: RecipientDetailsStore,
                                                    emailService: EmailService,
                                                    audit: Auditable,
                                                    dataStoreConnector: CustomsDataStoreConnector,
                                                    subscriptionDisplayConnector: SubscriptionDisplayConnector) {

  def onSubscriptionStatus(subscriptionComplete: SubscriptionComplete, formBundleId: String)(implicit hc: HeaderCarrier): Future[Any] = {
  ...
  private def sendAndStoreEmail(status: SubscriptionCompleteStatus, formBundleId: String)(implicit hc: HeaderCarrier) = {
    for {
      recipient <- recipientDetailsStore.recipientDetailsForBundleId(formBundleId)
      sendEmail <- emailService.sendEmail(recipient.recipientDetails, status)
      eoriNumber <- retrieveEori(recipient)
      updateDataStore <- Future.sequence(eoriNumber.map(dataStoreEmailRequest(recipient)).toList)
    } yield sendEmail
  }

  private def sendEmail(status: SubscriptionCompleteStatus, formBundleId: String)(implicit hc: HeaderCarrier) = {
    for {
      recipient <- recipientDetailsStore.recipientDetailsForBundleId(formBundleId)
      sendEmail <- emailService.sendEmail(recipient.recipientDetails, status)
    } yield sendEmail
  ...
  private def dataStoreEmailRequest(recipient: RecipientDetailsWithEori)(implicit hc: HeaderCarrier): String => Future[HttpResponse] = {
    eori =>
