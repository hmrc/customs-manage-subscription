...
class TaxEnrolmentsService @Inject()(appConfig: AppConfig, taxEnrolmentsConnector: TaxEnrolmentsConnector, recipientDetailsStore: RecipientDetailsStore) {
  ...
  def saveRecipientDetailsAndCallTaxEnrolment(formBundleId: String, recipientDetails: RecipientDetails, sapNumber: TaxPayerId, eori: Option[Eori], emailVerificationTimestamp: String, safeId: String)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Int] = {
    def saveRecipientDetails() = recipientDetailsStore.saveRecipientDetailsForBundleId(formBundleId, eori, recipientDetails, emailVerificationTimestamp, safeId)


    def taxEnrolmentsRequest = TaxEnrolmentsRequest(appConfig.taxEnrolmentsServiceName, s"${appConfig.taxEnrolmentsCallbackUrl}/$formBundleId", sapNumber.id)
