...
class RecipientDetailsRepository @Inject()(cacheRepo: RecipientDetailsCacheRepository) {
  ...
  def saveRecipientDetailsForBundleId(formBundleId: String, eori: Option[Eori], recipientDetails: RecipientDetails, emailVerificationTimestamp: String, safeId: String)(implicit writes: Writes[RecipientDetailsWithEori]): Future[Boolean] = {
    cacheRepo.createOrUpdate(Id(formBundleId), recipientDetailsKey, Json.toJson(RecipientDetailsWithEori(eori.map(_.value), recipientDetails, emailVerificationTimestamp, safeId))) map (_ => true)
  }

  def recipientDetailsForBundleId(formBundleId: String)(implicit reads: Reads[RecipientDetailsWithEori]): Future[Either[JsError, RecipientDetailsWithEori]] = {
    getCached(formBundleId)
  }
...
@Singleton
class RecipientDetailsCacheRepository @Inject()(sc: ServicesConfig, mongo: ReactiveMongoComponent)(implicit ec: ExecutionContext) extends
  CacheMongoRepository("recipient-details", sc.getDuration("cache.expiryInMinutes").toSeconds)(mongo.mongoConnector.db, ec)