...
@Singleton
class HandleSubscriptionController @Inject()(taxEnrolmentsService: TaxEnrolmentsService, cc: ControllerComponents, digitalHeaderValidator: DigitalHeaderValidator) extends BackendController(cc) {

  def handle(): Action[AnyContent] = digitalHeaderValidator async {
    implicit request =>
      request.body.asJson.fold(ifEmpty = Future.successful(ErrorResponse.ErrorGenericBadRequest.JsonResult)) { js =>
        js.validate[HandleSubscriptionRequest] match {
          case JsSuccess(r, _) =>
            taxEnrolmentsService.saveRecipientDetailsAndCallTaxEnrolment(r.formBundleId, r.recipientDetails, TaxPayerId(r.sapNumber), r.eori.map(Eori(_)), r.emailVerificationTimestamp, r.safeId).map {
              _ => NoContent
            }
          case JsError(_) =>
