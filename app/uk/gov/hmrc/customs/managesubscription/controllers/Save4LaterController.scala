...
@Singleton
class Save4LaterController @Inject()(save4LaterRepository: Save4LaterRepository, cc: ControllerComponents,override val authConnector: MicroserviceAuthConnector) extends BackendController(cc) with AuthorisedFunctions{
  ...
  def get(id:String,key:String): Action[AnyContent] =
    Action.async { implicit request =>
      authorised(AuthProviders(GovernmentGateway)) {
